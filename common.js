...
function calculateStandardMajor(subjects, rule) {
    ...
    // ▼▼▼ 「その他」科目の計算ロジックを修正 ▼▼▼
    let otherCredits = 0;
    const otherCap = rule.otherCap || 0;

    for (const subject of remainingSubjects) {
        if (rule.totalCap > 0 && totalCredits + subject.credits > rule.totalCap) continue;
        if (otherCap > 0 && otherCredits + subject.credits > otherCap) continue;

        otherCredits += subject.credits;
        totalCredits += subject.credits;

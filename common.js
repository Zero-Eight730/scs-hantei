...
function calculateScore(subjects, rule) {
    ...
    (rule.groups || []).forEach(group => {
        ...
        groupSubjects.forEach(subject => {
            // グループの上限と全体の単位数上限を超えない限り加算
            if ((!group.cap || group.cap === 0 || groupCredits + subject.credits <= group.cap) && 
                (!rule.totalCap || rule.totalCap === 0 || totalCredits + subject.credits <= rule.totalCap)) {
